...
const ApprovedDeopsits = () => {
    const hook = useHook();
    const location = useLocation();

    const [proofs, setProofs] = useState([]);

    const ApprovedDeopsits = async () => {
        try {
            const res = await axios.get(`${hook.endpoint}/admin/deposits/approved`);
            setProofs(res.data)
        } catch (error) {
            setProofs([]);
        }
    ...
    return (
        <div>
            <Helmet>
                <title>
                    Deposits | Administrator | The LIKEY
                </title>
            </Helmet>
...
export default ApprovedDeopsits
